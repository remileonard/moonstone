...
### 3.6 Sprites composites — rendu multi-CEL d'un chevalier

#### Principe général

Un chevalier en combat (ou tout autre combattant) occupe **une seule entrée**
dans la table des 40 acteurs IMAGEXCEL. Il n'existe pas d'acteur séparé pour
chaque partie du corps (tronc, jambes, épée). C'est le **script d'animation
de cet acteur unique** qui émet plusieurs instructions `IMAGEXCEL DRAW`
successives à chaque frame, chacune pointant vers un slot CEL différent, pour
assembler le personnage composite.

#### Chargement des fichiers `.ob` dans les slots CEL (`LAB_0116`)

`LAB_0116` (`mog.asm#L2429`) est appelée lors de l'initialisation d'un combat
impliquant un chevalier joueur (`LAB_0164` → `LAB_016F` → `LAB_0116`). Elle
charge trois fichiers dans trois slots consécutifs de la table `LAB_05E0` :

| Slot | Fichier    | Contenu                          |
|------|------------|----------------------------------|
| 0    | `He1.ob`   | Corps / torse du chevalier       |
| 1    | `He2.ob`   | Jambes                           |
| 2    | `He3.ob`   | Bras et épée                     |

```asm
LAB_0116:
    ; slot 0 → He1.ob
    LEA     LAB_0775,A0       ; "He1.ob"
    JSR     LAB_0CBB          ; charger + décompresser
    MOVE.L  A1,0(LAB_05E0)    ; ptr slot 0

    ; slot 1 → He2.ob  (LAB_0776)
    LEA     LAB_0776,A0       ; "He2.ob"
    JSR     LAB_0CBB
    MOVE.L  A1,4(LAB_05E0)    ; ptr slot 1

    ; slot 2 → He3.ob  (LAB_0777)
    LEA     LAB_0777,A0       ; "He3.ob"
    JSR     LAB_0CBB
    MOVE.L  A1,8(LAB_05E0)    ; ptr slot 2
```

Pour un ennemi chevalier (`LAB_011A` → `LAB_011B`), seuls `He1.ob` (slot 0)
et `He2.ob` (slot 1) sont chargés (les ennemis n'ont pas de script d'épée
distinct).

#### Affectation des scripts d'animation (`LAB_0167`)

La routine `LAB_0167` (`mog.asm#L3575`) remplit les pointeurs de scripts dans
la structure de l'acteur (registre A1) :
...
| `34`   | `LAB_05F5`     | Attaque                     |
| `38`   | `LAB_05E1`     | Script courant (état repos) |
| `42`   | `LAB_05F7`     | Mort                        |
| `46`   | `LAB_0610`     | Chute                       |
| `50`   | `LAB_05F8`     | Dégâts reçus                |

Chaque script est une séquence d'instructions IMAGEXCEL (format 6 octets
décrit en §3.2 de `DOC_MOTEUR_IMAGEXCEL.md`).

#### Structure d'un script composite : exemple `LAB_07FC`

`LAB_07FC` (`mog.asm#L14234`) est le script d'animation par défaut passé à
`LAB_01A9` lors de l'initialisation (`MOVEA.L #LAB_07FC,A0`). Il enchaîne
pour chaque frame plusieurs instructions DRAW couvrant les trois slots :

```
; Fragment d'une frame de LAB_07FC (bytes bruts)
...
Le décodage de l'opcode suit la règle `slot = opcode >> 2` :
- `$00` → slot 0 (`He1.ob`)
- `$04` → slot 1 (`He2.ob`)
- `$08` → slot 2 (`He3.ob`)
- `$0C` → slot 3 (ex. `dw1.cel` ou sprite d'arme supplémentaire)
...
Chaque frame d'animation, l'interpréteur IMAGEXCEL (`LAB_01F1`,
`program.asm#L4208`) parcourt le script et dessine les trois morceaux l'un
après l'autre, donnant l'illusion d'un personnage composite animé.
