...
static MoonCel *cel_decode(const uint8_t *buf, size_t len)
{
    ...
    uint32_t data_offset = ((uint32_t)buf[2] << 24) | ((uint32_t)buf[3] << 16) |
    ...
    const uint8_t *comp_data = buf + data_offset;
    size_t comp_len = len - data_offset;

    /* Estimate decompressed size: worst case ~3× */
    size_t decomp_max = comp_len * 4 + 65536;
    uint8_t *pixels = (uint8_t *)malloc(decomp_max);
    if (!pixels)
        return NULL;

    int decomp_len = moon_lzss_decompress(comp_data, comp_len, pixels, decomp_max);
    if (decomp_len < 0) {
        free(pixels);
        return NULL;
    }
    ...
    for (int i = 0; i < (int)frame_count; i++) {
        ...
        if (frame_off + frame_size > (size_t)decomp_len) {
            /* Can happen for large CELs; clamp gracefully */
